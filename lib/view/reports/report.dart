...
// Controller Method - Fixed Version
...
_printReport(controller);
  ...
  Widget _buildReportTypeDropdown(ReportsController controller) {
    final reportTypes = [
      if(user?.groupId==2)
      {"value": "branch_per_month", "label": "تقرير الفروع شهرياً"},
            if(user?.groupId==2)

      {"value": "branch_total", "label": "إجمالي تقرير الفروع"},
            if(user?.groupId==2)

      {"value": "technology_per_month", "label": "تقرير التكنولوجيا شهرياً"},
            if(user?.groupId==2)

      {"value": "technology_total", "label": "إجمالي تقرير التكنولوجيا"},
            if(user?.groupId==3)

      {"value": "station-bills", "label": "فواتير المحطات"},
      
            if(user?.groupId==3)

      {"value": "water-techs-3-month", "label": "تقرير المياه (3 أشهر)"},
                  if(user?.groupId==3)

      {"value": "sanity-techs-3-month", "label": "تقرير الصرف (3 أشهر)"},
    ];
  ...
  List<DataColumn> _buildOptimizedColumns(ReportsController controller) {
    ...
    if (reportType != "station-bills"&&reportType != "sanity-techs-3-month"&&reportType !=  "water-techs-3-month") {
      columns.addAll([
  ...
  List<DataCell> _buildCellsForBranch(ReportBranch branch, String reportType) {
    ...
    // Add other data if not station bills
    if (reportType != "station-bills"&&reportType != "sanity-techs-3-month"&&reportType !=  "water-techs-3-month") {
      cells.addAll([
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text(
            'قم بتحديد التواريخ ونوع التقرير للبحث عن البيانات',
            style: TextStyle(
              fontSize: 16,
              color: Colors.grey.shade500,
            ),
...
.info-section > * {
  ...
  <body>
    <div class="print-container">
      ...
      <div class="footer">
        <p>تم إنشاء هذا التقرير تلقائياً بواسطة نظام إدارة الطاقة | ${DateTime.now().year}</p>
      </div>
    </div>
  </body>
  </html>
...
String _generateTableHeaders(String reportType) {
  ...
  headers.add('<th>قيمة المطالبة</th>');
     ...
     if (reportType != "station-bills"&&reportType != "water-techs-3-month"&&reportType != "sanity-techs-3-month") {
   ...
   if (reportType.contains("techs-3")) {
    headers.addAll(['<th>كمية المياه</th>', '<th>كمية الكهرباء</th>',
     '<th>النسبة</th>']);
...
String _generateTableRows(ReportsController controller) {
  String regularRows = controller.branchs.map((branch) {
    ...
    // Claim value column
    cells.add('<td class="highlight-cell">${branch.totalBill}</td>');
    
    // Additional columns for non-station-bills reports
    if (reportType != "station-bills"&&reportType != "water-techs-3-month"&&reportType != "sanity-techs-3-month") {
      cells.addAll([
