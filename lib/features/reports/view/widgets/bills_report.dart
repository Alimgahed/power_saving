  ...
  List<DataRow> buildRows() {
    return controller.filteredBranchs.map((b) {
      return DataRow(cells: [
        ...
        dataCell('${b.month}'),
        dataCell('${b.year}'),
        styledCell(b.stationnames?.isNotEmpty == true ? b.stationnames! : (b.stationname ?? ""), Colors.blueAccent),
...
class BillsReportPrinter extends BaseReportPrinter {
  ...
  List<String> rowCells(ReportBranch b, String type) {
    ...
    return [
      ...
      '<td>${b.month}</td>',
      '<td>${b.year}</td>',
      '<td>${b.stationnames?.isNotEmpty == true ? b.stationnames : (b.stationname?.isNotEmpty == true ? b.stationname : "لا يوجد")}</td>',
