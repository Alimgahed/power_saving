...
class Reports extends StatelessWidget {
  ...
  List<DataColumn> _buildOptimizedColumns(ReportsController controller) {
    ...
    if (reportType == ("bills")) {
      columns.addAll([
                        _buildColumn("المحطات", Icons.location_on_outlined),
  ...
  List<DataCell> _buildCellsForBranch(ReportBranch branch, String reportType) {
    ...
    if (reportType.contains("branch")) {
      ...
      cells.add(_buildStyledCell(branch.stationname ?? "", Colors.green));
            ...
            cells.add(_buildStyledCell(branch.accountnumber ?? "", Colors.blueAccent));
    ...
    if (reportType == ("bills")) {
      cells.addAll([
        _buildStyledCell(branch.station ?? "", Colors.blueAccent),
  ...
  String _generateTableHeaders(String reportType) {
    ...
    if (reportType == "bills") {
      headers.addAll([  '<th>المحطات</th>','<th>مسدد</th>', '<th>شهر الترحيل</th>', '<th>سنة الترحيل</th>']);
    }
